...
const center_canvas = new Vec2(canvas.width/2, canvas.height/2);
const center_canvas_dist = center_canvas.length();
...
class Boid {
    ...
    find_frens(flock) {
        ...
        for (let x in flock) {
            let boid = flock[x];
            let in_range = boid.pos.subtract(this.pos).length_sq() <= group_radius_sq;
    ...
    update(flock) {
        this.border();
        this.in_pad = this.in_padding();
        let frens = this.find_frens(flock);
        let total_vecs = [this.vel];
        if (frens.length != 0 && !this.in_pad) {
            let rule_scale = speed * rule_vec_factor
            let heading = Vec2.sum(frens.map((b) => b.vel)).scale(rule_scale);
            let center = Vec2.sum(frens.map((b) => b.pos)).scalar(1/frens.length);
            let normal_dist = Math.min(center.subtract(this.pos).length()/group_radius, 1);
            let dir_to_center = center.subtract(this.pos).scale(rule_scale);

            let rule_vecs = [
                heading.scalar(alignment * normal_dist),
                dir_to_center.scalar(cohesion * normal_dist),
                dir_to_center.scalar(-(separation * (1 - normal_dist)))
            ];
